...
function swSetConfig(c) {
  ...
  var charType = el(c, "char_type").value;
  ...
  cfg.char_type = (svcType == 3) ? parseInt(el(c, "char_type").value) : -1;
...
function updateComponent(cd) {
  ...
  switch (cd.type) {
    ...
    case 2: // Lock
      ...
      if (cd.svc_type == 3) {
        selectIfNotModified(el(c, "char_type"), cd.char_type);
        el(c, "char_type_container").style.display = "block";
        el(c, "char_type_label").innerText = "Valve Type:";
      } else {
        el(c, "char_type_container").style.display = "none";
      }
