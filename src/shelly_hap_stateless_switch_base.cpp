...
StatelessSwitchBase::StatelessSwitchBase(int id, Input *in,
    ...
    : Component(id),
      Service(
          ...
          (iid_base + (SHELLY_HAP_IID_STEP_STATELESS_SWITCH * (id - 1))), type,
          debug_description),
      in_(in),
      cfg_(cfg) {
}

StatelessSwitchBase::~StatelessSwitchBase() {
  in_->RemoveHandler(handler_id_);
}

Component::Type StatelessSwitchBase::type() const {
  return Type::kStatelessSwitch;
}

std::string StatelessSwitchBase::name() const {
  return cfg_->name;
}

Status StatelessSwitchBase::Init() {
  if (in_ == nullptr) {
  ...
  handler_id_ = in_->AddHandler(
      std::bind(&StatelessSwitchBase::InputEventHandler, this, _1, _2));

  return Status::OK();
}

StatusOr<std::string> StatelessSwitchBase::GetInfo() const {
...
StatusOr<std::string> StatelessSwitchBase::GetInfoJSON() const {
  double last_ev_age = -1;
  if (last_ev_ts_ > 0) {
    last_ev_age = mgos_uptime() - last_ev_ts_;
  }
...
Status StatelessSwitchBase::SetConfig(const std::string &config_json,
...
Status StatelessSwitchBase::SetState(const std::string &state_json) {
  (void) state_json;
  return Status::UNIMPLEMENTED();
}

void StatelessSwitchBase::InputEventHandler(Input::Event ev, bool state) {
