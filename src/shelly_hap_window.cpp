...
Status Window::Init() {
  ...
  tgt_pos_char_ = new mgos::hap::UInt8Characteristic(
      ...
      [this](HAPAccessoryServerRef *,
        ...
        mgos::InvokeCB(std::bind(&Window::HAPSetTgtPos, this, value));
        return kHAPError_None;
      },
  ...
  switch (static_cast<InMode>(cfg_->in_mode)) {
    ...
    case InMode::kSingle:
      in_open_handler_ = in_open_->AddHandler(
          std::bind(&Window::HandleInputEvent2, this, _1, _2));
      break;
...
Component::Type Window::type() const {
  return Type::kWindow;
}
...
StatusOr<std::string> Window::GetInfo() const {
...
StatusOr<std::string> Window::GetInfoJSON() const {
...
Status Window::SetState(const std::string &state_json) {
...
// static
const char *Window::StateStr(State state) {
  switch (state) {
    case State::kNone:
      return "none";
    case State::kIdle:
...
// static
float Window::TrimPos(float pos) {
  if (pos < kFullyClosed) {
    pos = kFullyClosed;
...
void Window::SetInternalState(State new_state) {
...
void Window::SetCurPos(float new_cur_pos, float p) {
...
void Window::SetTgtPos(float new_tgt_pos, const char *src) {
...
void Window::HAPSetTgtPos(float value) {
...
Window::Direction Window::GetDesiredMoveDirection() {
...
void Window::Move(Direction dir) {
...
void Window::HandleInputEvent2(Input::Event ev, bool state) {
  if (!cfg_->calibrated) {
    HandleInputEventNotCalibrated();
    return;
  }
...
void Window::HandleInputEventNotCalibrated() {
