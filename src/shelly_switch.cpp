...
StatusOr<std::string> ShellySwitch::GetInfoJSON() const {
  std::string res = mgos::JSONPrintStringf(
      "{id: %d, type: %d, name: %Q, svc_type: %d, char_type: %d, in_mode: %d, "
      "in_inverted: %B, initial: %d, state: %B, auto_off: %B, "
      "auto_off_delay: %.3f",
      id(), type(), (cfg_->name ? cfg_->name : ""), cfg_->svc_type, cfg_->char_type,
...
Status ShellySwitch::SetConfig(const std::string &config_json,
  ...
  json_scanf(config_json.c_str(), config_json.size(),
             "{name: %Q, svc_type: %d, char_type: %d, in_mode: %d, in_inverted: %B, "
             "initial_state: %d, "
             "auto_off: %B, auto_off_delay: %lf}",
             &cfg.name, &cfg.svc_type, &cfg.char_type, &cfg.in_mode, &in_inverted,
  ...
  if ((cfg.svc_type != 3 && cfg.char_type != -1) ||
      (cfg.svc_type == 3 && cfg.char_type < 0) ||
      (cfg.svc_type == 3 && cfg.char_type > 3)) {
    return mgos::Errorf(STATUS_INVALID_ARGUMENT, "invalid %s", "char_type");
  }
  if (cfg.in_mode != -2 &&
  ...
  if (cfg_->char_type != cfg.char_type) {
    cfg_->char_type = cfg.char_type;
    *restart_required = true;
  }
